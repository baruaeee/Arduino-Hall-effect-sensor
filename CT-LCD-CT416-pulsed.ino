...
int sub_menu = 1;     // to count and nevigate submenu
...
void updateMenu() {
  switch (menu) {
    ...
    case 3:
      ...
      break;
    case 4:
      menu = 3;               // as there are only 2 options if menu increamented to 3 it will freeze to second option
      break;
  }
}
...
void StartAuto(){
  ...
  gate = 90;
  float U1=(float)analogRead(A1)*5/1024;                  // Analog input(10-bit), 0~5 volt = 0~1023
  float U2=(float)analogRead(A2)*5/1024;
  float I1=10*(U1-0.68);                                  // this is an approximate formula to convert the voltage to equivalent current
  float I2=10*(U2-0.68);
  lcd.clear();
  while(key != 'D'){                                      // unless pressing 'S', while loop continues
    ...
    lcd.setCursor(0,1);
    lcd.print("U1:"+ String(U1) + " U2:" + String(U2));
    //lcd.print("I1:"+ String(I1) + " I2:" + String(I2));   // to show the correspinding current
    //lcd.setCursor(11, 0);
    //lcd.print("Dwn-Ex");                                   // to remind pressing select to exit anytime
    //delay(200);
    ...
    if(FileAuto){
      FileAuto.println("Gate, U1, I1, U2, I2");                   // write header
    }
    else {
    ...
    delay(1000);
    float U1=(float)analogRead(A1)*5/1024;                // this is the 10-bit analog value of analog input
    float U2=(float)analogRead(A2)*5/1024;
    float I1=10*(U1-0.68);
    float I2=10*(U2-0.68);
    if(I1>(float)Imax){x=0;}                               // exit loop when current is greater then Imax ampere
		//delay(200);
    lcd.clear();        
    lcd.setCursor(0,0);
    ...
    lcd.setCursor(0,1);
    //lcd.print("U1:"+ String(U1) + " U2:" + String(U2));
    lcd.print("I1:"+ String(I1) + " I2:" + String(I2));
    if(FileAuto){
      FileAuto.println(String((float)gate*5/256)+ ", "+ String(U1) + ", "+ String(I1) + ", " + String(U2) + ", " + String(I2));
    }
    else {
      Serial.println("error opening Auto.txt");
      FileAuto.close();
    ...
    if(165%step!=0 && gate==(255-165%step)){            // to keep last gate value equal to 255 
      gate=255-step;
		...
		else if(checkButton()=='D'){key = 'D';}
    delay(2000);             // exit operation if 'S' button pushed
    analogWrite(ctrl, 0);
    delay(500);
	}
...
void StartManual(){
  char key = ' ';
  gate = 90;
  float U1=(float)analogRead(A1)*5/1024;
  float U2=(float)analogRead(A2)*5/1024;
  float I1=10*(U1-0.68);
  float I2=10*(U2-0.68);
  //lcd.clear();
  ...
  if(FileManual){
    FileManual.println("Gate, U1, I1, U2, I2");
  }
  else {
  ...
  while(key != 'D'){
    ...
    if(checkButton()=='R'){
      ...
      lcd.print("I1:"+ String(I1) + " I2:" + String(I2));
      if(FileManual){
        FileManual.println(String((float)gate*5/256)+ ", "+ String(U1) + ", "+ String(I1) + ", " + String(U2) + ", " + String(I2));
      }
      else {
        Serial.println("error opening Manual.txt");
        FileManual.close();
      ...
      //lcd.print("I1:"+ String(I1) + " I2:" + String(I2));
      if(FileManual){
        FileManual.println(String((float)gate*5/256)+ ", "+ String(I1) + ", " + String(I2));
      }
      else {
        Serial.println("error opening Manual.txt");
        FileManual.close();
